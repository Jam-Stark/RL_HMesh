...
namespace HMeshLib
{
	...
	{		
		...
		for (int i = 0; i < 6; i++)
		{
			...
			for (int adjFIndex = 0; adjFIndex < facev->neighbor_fs.size(); adjFIndex++)
			{
				...
				{				
					...
					hexFs[i] = adjF;//ùùù?ùùùù?ùfaceùùùùfsùù
					adjF->neighbor_hs.push_back(h->id());
					
					//ùùùùù?ùùùù?ùùùùù?ùùùù?ù?ùùùhexEsùù
					if (i<4)
					{
		...
		{		
			...
			if (hexf->es.size()==4)//?ùùùùùùùΩùùùùùùù
			{
				continue;
			}				
			for (int eIndex = 0; eIndex < 4; eIndex++)
